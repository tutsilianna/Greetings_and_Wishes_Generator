...
class GreetingParse():
    ...
    def __init__(self) -> None:
        self.url   = "https://www.pozdravuha.ru/"
        ...
        self.pages = '.btn.btn-default.paginator_a' # also used in function next_page()
    ...
    def count_pages(self) -> None:
        items = self.page.query_selector_all(self.pages)
    ...
    def get_data(self) -> None:
        ...
        for x in data_dict:
            self.page.goto(data_dict[x], timeout=60000)
            time.sleep(2)
            data_dict[x] = self.parse_menu(i=1)
            if isinstance(data_dict[x], dict):
                for y in data_dict[x]:
                    self.page.goto(data_dict[x][y], timeout=60000)
                    time.sleep(2)
